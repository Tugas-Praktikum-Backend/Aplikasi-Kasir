...
async function startShift(req, res, next) {
    try {
        const employeeId = req.body.employeeId || new mongoose.Types.ObjectId().toString();

        const activeShift = await db.findOne({ employeeId: employeeId, endTime: null });
        if (activeShift) {
...
async function endShift(req, res, next) {
    try {
        ...
        if (!employeeId) {
            throw Error('employeeId is required to end the shift');
        }

        const result = await db.findOneAndUpdate(
            { employeeId: employeeId, endTime: null }, 
