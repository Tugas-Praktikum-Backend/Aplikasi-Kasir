...
async function addCustomer(req, res, next){
    try {
        ...
        res.status(201).json({ message: `Successfully added new customer. Customer added: ${result.customerName}` });
    } catch(err){
        next(err);
    }
}
...
async function topUp(req, res, next){
    try{
        ...
        const updatedPayment = updated.paymentMethod.find(p => p.provider === provider);

        if(result.matchedCount === 0){
            throw Error('Customer or payment not found');
        }
        
        res.status(200).json({ message: 'Top up success', balance: updatedPayment.balance });
    }catch(err){
        next(err)
    }
}
